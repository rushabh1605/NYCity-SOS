// Nandani's Alert Display Screen Logic (Hiren's copy)
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderAlert(alertData) {
    ...
    alertBanner.textContent = `ALERT DISPATCHED — ${alertData.urgency.toUpperCase()} URGENCY`;
    ...
    peopleCount.textContent = alertData.people_present ?? 0;
  ...
  function updateStopwatch() {
    if (!alertTime) return;
    const now = new Date();
    const elapsedSeconds = Math.floor((now - alertTime) / 1000);
    ...
    sse.onmessage = (e) => {
      try {
        ...
        if (data && data.alert_id) {
          console.log('Received SSE alert:', data);
          renderAlert(data);
        }
      } catch (parseErr) {}
    };
